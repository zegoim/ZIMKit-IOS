...
- (void)layoutSubviews {
    ...
    if (self.coverViews) {
        ...
        self.downloadBtn.frame = CGRectMake(16, self.contentView.height - 44-8.5-Bottom_SafeHeight, self.contentView.width - 16*2, 44);
    }
...
- (void)downloadAction {
    if ([self.downloadBtn.titleLabel.text isEqualToString:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_download_image"]]) {
        if ([self.delegate respondsToSelector:@selector(photoBrowser:onDwonloadBtnClick:image:photo:)]) {
            GKPhoto *curPhoto = self.photos[self.currentIndex];
            [self.delegate photoBrowser:self onDwonloadBtnClick:self.currentIndex image:self.curPhotoView.imageView.image photo:curPhoto];
        }
    } else {
//        if (self.isAppeared) return;
        self.isAppeared = YES;
    ...
    for (NSInteger i = index - 1; i <= index + 1; i++) {
        ...
        if (photoView == nil) {
            ...
            photoView.loadFailed = ^(GKPhotoView * _Nonnull curPhotoView) {
                ...
                if (self.loadStyle == GKPhotoBrowserLoadStyleKit) {
                    weakSelf.downloadBtn.hidden = NO;
                    [weakSelf.downloadBtn setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_redownload_image"] forState:UIControlStateNormal];
                }
                
            };
            
            photoView.loadComplete = ^(BOOL scuess) {
                if (self.loadStyle == GKPhotoBrowserLoadStyleKit) {
                    weakSelf.downloadBtn.hidden = NO;
                    if (scuess) {
                        [weakSelf.downloadBtn setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_download_image"] forState:UIControlStateNormal];
                    } else {
                        [weakSelf.downloadBtn setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_redownload_image"] forState:UIControlStateNormal];
                    }
                }
...
- (UIButton *)downloadBtn {
    if (!_downloadBtn) {
        ...
        [_downloadBtn setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_download_image"] forState:UIControlStateNormal];
        [_downloadBtn setTitleColor:[UIColor dynamicColor:ZIMKitHexColor(0xFFFFFF) lightColor:ZIMKitHexColor(0xFFFFFF)] forState:UIControlStateNormal];
        _downloadBtn.hidden = YES;
        _downloadBtn.layer.cornerRadius = 12.0;
        _downloadBtn.backgroundColor = [UIColor dynamicColor:ZIMKitHexColor(0x3478FC) lightColor:ZIMKitHexColor(0x3478FC)];
        _downloadBtn.titleLabel.font = [UIFont systemFontOfSize:16.0 weight:UIFontWeightSemibold];
...
- (ZIMKitLoadingView *)loadingImageView {
    if (!_loadingImageView) {
        _loadingImageView = [[ZIMKitLoadingView alloc] initWithFrame:CGRectZero];
        _loadingImageView.center = CGPointMake(self.contentView.bounds.size.width * 0.5, self.contentView.bounds.size.height * 0.5);
